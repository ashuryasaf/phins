...
| At rest | `national_id_hash` (HMAC-SHA256 keyed by `PHINS_IDENTITY_HASH_KEY`, else an existing keyring `identity-hash` key, else the raw `PHINS_ENCRYPTION_KEY` as before the keyring, else a minted ring key), `national_id_last4`, `nationality`, `identity_captured_at`, `identity_source`, `identity_history`; the number itself only in `national_id_encrypted` (Fernet vault). Keys are **one per purpose, many records**: `security/keyring.py` uses `PHINS_ENCRYPTION_KEY` when set and otherwise mints a durable key once into `platform_keys` (DB mode) or `PHINS_KEYRING_PATH`, shared by all replicas and never replaced; decryption tries every known key, so introducing an explicit key later never orphans a blob. Only when no key can be resolved at all does the service refuse to store (503 `identity_vault_unavailable`) unless `PHINS_TEST_MODE` / `PHINS_IDENTITY_ALLOW_PLAINTEXT_VAULT`. |
