...
|---|---|---|---|---|---|
| 1 | **DI-1** Eliminate split-brain stores | Integrity (prereq) | Medium | Makes integrity true | `cursor/di-store-handles-ac1a` |
| 2 | **BI-1** One canonical KPI module | BI | Low | Yes (pure fns) | `cursor/bi-canonical-kpis-ac1a` |
| 3 | **BI-2** Activate dashboard cache | BI | Low | Yes (read-only) | `cursor/bi-activate-cache-ac1a` |
| 4 | **AI-1** Persist every AI decision | AI (keystone) | Low | Yes (append-only) | `cursor/ai-decision-log-ac1a` |
| 5 | **BI-4** Continuous integrity + history | BI/Integrity | Medium | Strengthens it | `cursor/bi-integrity-timeseries-ac1a` |
| 6 | **AI-2** Weekly threshold calibration | AI | Low–Med | Yes (config only) | `cursor/ai-threshold-calibration-ac1a` |
...
| 12 | **AI-3** Versioned model + registry | AI | Medium | Yes (scores only) | `cursor/ai-model-registry-ac1a` |
...
## 8. What this review intentionally did NOT change

Per the engagement, **no platform code was modified**. No services, routes,
schemas, ledgers, or engines were touched; the running system's behavior and data
integrity are unchanged by this document. This file is a review artifact and a set
of PR-ready specifications. Each item in §4, §5, and §6 is scoped so it can be
implemented as its own focused PR — and each is gated so that adding AI/BI
intelligence never compromises the platform's core invariant: **every balance and
every decision is reproducible from append-only, hash-chained state, and derived
intelligence may recommend but never post.**
