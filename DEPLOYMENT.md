...
| `PHINS_IDENTITY_HASH_KEY` | keyring | Optional explicit HMAC key for the lookup hash (`national_id_hash`). When unset the key is, in order: an existing keyring `identity-hash` key, else the raw `PHINS_ENCRYPTION_KEY` (the pre-keyring behaviour, so existing hashes keep matching; the env key is never copied into the keyring), else a ring key minted once and never replaced. Must stay stable for the life of the data — changing or removing whichever key is in effect orphans uniqueness checks and every pipeline reference. |
