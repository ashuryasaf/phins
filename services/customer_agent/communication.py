"""
PHINS Customer Communication Agent (B6 facade)
==============================================
...
B6 wiring shared with :mod:`services.customer_agent.service_desk`:

- every send is written to the shared :class:`InteractionLog` **before** the
  provider is called (``pending``) and finalised afterwards (``sent`` /
  ``failed`` / ``refused``), so a crash mid-send leaves a visible row;
- WhatsApp sends pass through :class:`MessagingPolicy` (per-customer channel
  consent + daily cap). Welcome / bill / reminder copy is *transactional*
  (only an explicit revocation blocks it); ``message`` / ``offer`` copy is
  *relational* and needs consent on file;
- an OTP verified against the customer's WhatsApp number is recorded as
  ``otp_verified`` consent for that channel, so the evidence is kept;
- :meth:`CustomerCommunicationAgent.escalate` goes through the shared
  :class:`EscalationDesk`.

``services.customer_communication_agent`` remains as an import shim.
"""
...
class CustomerCommunicationAgent:
    ...
    def __init__(self, notification_service=None, *, interaction_log: Optional[InteractionLog] = None,
    ...
    # ------------------------------------------------------------------
    # Shared B6 infrastructure (lazy so construction stays cheap)
    # ------------------------------------------------------------------
    ...
    def interactions_for(self, customer_id: str, **kw) -> List[Interaction]:
        """This customer's timeline from the shared log (both facades)."""
        return self.interaction_log.for_customer(customer_id, **kw)

    @instrument_agent('customer_communication', decision_fn=lambda r: 'escalated')
    def escalate(self, *, customer_id: str, reason: str, assigned_team: str = 'Service Team',
                 actor: Optional[str] = None, customer_name: Optional[str] = None,
                 context: Optional[Dict[str, Any]] = None) -> Escalation:
        """Hand a customer to a human team through the shared escalation desk."""
        return self.escalation_desk.escalate(
            customer_id=customer_id, reason=reason, source_agent=AGENT_NAME,
            assigned_team=assigned_team, actor=actor, customer_name=customer_name,
            context=context,
        )
    ...
    def _logged_send(
        ...
        *,
        ...
        """Consent/cap gate + pending->final interaction row around one send.

        Returns the notification result dict. A refusal is returned in the same
        shape (``success=False``) with ``error_code`` set to the policy code so
        ...
        if not decision.allowed:
            self.interaction_log.record(
                customer_id=request.customer_id or '', agent=AGENT_NAME, kind=kind,
                channel=channel, status='refused', template=template, purpose=purpose,
                recipient=request.recipient, actor=actor, code=decision.code,
                detail=decision.reason, metadata={**(metadata or {}), 'policy': decision.to_dict()},
            )
            return {
                'success': False,
        ...
        try:
            ...
            self.interaction_log.update(row.interaction_id, status='failed',
                                        code='SEND_EXCEPTION', detail=str(exc)[:300])
            raise
        ...
        result_dict['interaction_id'] = row.interaction_id
        return result_dict
...
""".strip()
    ...
    def send_welcome_package(
        ...
        *,
        ...
        otp_verified_identifier: Optional[str] = None
        if require_otp_validation:
            ...
            otp_verified_identifier = otp_identifier_value
        ...
        if otp_verified_phone and otp_verified_phone == self._normalize_phone(whatsapp_phone):
            try:
                self.messaging_policy.registry.set(
                    ...
                    actor=actor or 'customer', note=f'OTP {otp_verification_type.value}',
                )
            except Exception:  # consent capture never blocks the welcome package
                pass
        ...
        log_meta = {'otp_validated': bool(require_otp_validation)}
        email_result_dict = self._logged_send(
            email_request, kind='welcome', template='welcome', purpose=TRANSACTIONAL,
            actor=actor, customer_record=customer_record, metadata=log_meta,
        )
        ...
        if whatsapp_phone:
            ...
            whatsapp_result_dict = self._logged_send(
                whatsapp_request, kind='welcome', template='welcome', purpose=TRANSACTIONAL,
                actor=actor, customer_record=customer_record, metadata=log_meta,
            )

        success = bool(email_result_dict.get("success") and (whatsapp_result_dict is None or whatsapp_result_dict.get("success")))
        return {
            "success": success,
            "customer_id": customer_id,
            "report": report_dict,
    ...
    def send_customer_outreach(
        ...
        *,
        ...
        ``customer_record`` (the stored record) lets the messaging policy honour
        consent flags kept on it; explicit grants/revocations in the consent
        registry take precedence either way. WhatsApp copy for ``message`` /
        ``offer`` is relational and needs consent; ``bill`` / ``reminder`` /
        ``welcome`` is transactional (blocked only by an explicit opt-out).
        """
        ...
        whatsapp_result_dict: Optional[Dict[str, Any]] = None
        purpose = TEMPLATE_PURPOSE.get(template_key, RELATIONAL)
        log_meta = {"offers_count": len(offer_summaries)}
        ...
        if send_email:
            ...
            email_result_dict = self._logged_send(
                email_request, kind='outreach', template=template_key, purpose=purpose,
                actor=actor, customer_record=customer_record, metadata=log_meta,
            )
        ...
        if send_whatsapp:
            ...
            whatsapp_result_dict = self._logged_send(
                whatsapp_request, kind='outreach', template=template_key, purpose=purpose,
                actor=actor, customer_record=customer_record, metadata=log_meta,
            )
        ...
        refused = [
            r for r in (email_result_dict, whatsapp_result_dict)
            if r is not None and r.get("status") == "refused"
        ]
        result: Dict[str, Any] = {
            "success": success,
            "customer_id": customer_id,
        ...
        if refused and not success:
            # Surface the policy refusal at the top level in the legacy error shape.
            result["error"] = refused[0].get("error_message") or "Message refused by messaging policy"
            result["code"] = refused[0].get("error_code") or "MESSAGE_REFUSED"
        return result
...
def get_customer_communication_agent(notification_service=None, **kw) -> CustomerCommunicationAgent:
    """Factory helper for customer communication agent."""
    return CustomerCommunicationAgent(notification_service=notification_service, **kw)


def _communication_health() -> Dict[str, Any]:
    from .consent import consent_enforced, daily_cap, get_consent_registry
    return {
        'status': 'ok',
...
try:
    ...
    _register_agent(_AgentDescriptor(
        ...
        description=(
            ...
            'sensitive notifications. Sends are consent- and cap-gated and '
            'recorded in the shared customer interaction log.'
        ),
    ...
    ), health_fn=_communication_health)
...
__all__ = [
    'AGENT_NAME', 'CustomerCommunicationAgent', 'ExecutiveReport',
