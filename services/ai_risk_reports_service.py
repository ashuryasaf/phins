...
• Data completeness: {completeness}%"""
    ...
    def _generate_pension_section(self, pension_data: Dict[str, Any], 
        ...
        Uses data from the PensionDataAgent for Hebrew pension/insurance XML files.
        
        This displays:
        - Pension report generated by PensionDataAgent
        ...
        # If we have the pre-generated pension report, include it
        if pension_report:
        ...
        if pension_data:
            ...
            if is_hebrew:
                ...
                content_lines.append(f"• סה״כ יתרה בחשבונות: {summary.get('total_balance_formatted', '₪0')}")
                content_lines.append(f"• סה״כ פיצויים צבורים: {summary.get('total_severance_formatted', '₪0')}")
                content_lines.append(f"• מספר חשבונות/פוליסות: {summary.get('account_count', 0)}")
                content_lines.append(f"• מספר יצרנים/חברות: {summary.get('provider_count', 0)}")
                ...
                content_lines.append(f"• סטטוס: {summary.get('section14_status', 'לא ידוע')}")
                ...
                if trend:
                    trend_he = summary.get('contribution_trend_he', trend)
                    content_lines.append("📈 מגמת הפקדות:")
                    content_lines.append(f"• מגמה: {trend_he}")
                    content_lines.append(f"• סה״כ הפקדות עובד: ₪{summary.get('total_employee_contributions', 0):,.2f}")
                    content_lines.append(f"• סה״כ הפקדות מעסיק: ₪{summary.get('total_employer_contributions', 0):,.2f}")
                    content_lines.append(f"• סה״כ הפקדות פיצויים: ₪{summary.get('total_severance_contributions', 0):,.2f}")
                    content_lines.append("")
                ...
                if accounts:
                    ...
                    for i, acct in enumerate(accounts[:10], 1):
                        ...
                        if acct.get('product_name') or acct.get('product_type'):
                            content_lines.append(f"   • מוצר: {acct.get('product_name', acct.get('product_type', 'לא ידוע'))}")
                        if acct.get('status'):
                            content_lines.append(f"   • סטטוס: {acct.get('status')}")
                        content_lines.append(f"   • יתרה: ₪{acct.get('balance', 0):,.2f}")
                        if acct.get('severance_balance', 0) > 0:
                            content_lines.append(f"   • פיצויים: ₪{acct.get('severance_balance', 0):,.2f}")
                        if acct.get('employer'):
                            emp = acct['employer']
                            if isinstance(emp, dict):
                                content_lines.append(f"   • מעסיק: {emp.get('name', '')}")
                ...
                content_lines.append("=" * 40)
                content_lines.append(f"• Total Account Balance: {summary.get('total_balance_formatted', '₪0')}")
                content_lines.append(f"• Total Severance Accrued: {summary.get('total_severance_formatted', '₪0')}")
                content_lines.append(f"• Number of Accounts/Policies: {summary.get('account_count', 0)}")
                content_lines.append(f"• Number of Providers: {summary.get('provider_count', 0)}")
                ...
                content_lines.append(f"• Covered: {'Yes' if summary.get('section14_any') else 'No'}")
                if summary.get('section14_any'):
                ...
                if trend:
                    ...
                    content_lines.append(f"• Total Employee Contributions: ₪{summary.get('total_employee_contributions', 0):,.2f}")
                    content_lines.append(f"• Total Employer Contributions: ₪{summary.get('total_employer_contributions', 0):,.2f}")
                    content_lines.append(f"• Total Severance Contributions: ₪{summary.get('total_severance_contributions', 0):,.2f}")
                    content_lines.append("")
